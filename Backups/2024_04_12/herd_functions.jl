...
#     @cuda threads=length(Boids) update_kernel!(d_i|>cu, d_r|>cu, d_τ|>cu, d_ϕ|>cu, d_cn|>cu, d_sx|>cu, d_rr|>cu, d_pos|>cu, d_angle|>cu, d_model)#, d_speed, d_neighbors, d_params)
    @cuda threads=length(Boids) update_kernel!(d_i|>cu, d_r|>cu, d_θ|>cu, d_τ|>cu, d_ϕ|>cu, d_cn|>cu, d_sx|>cu, d_rr|>cu, d_coords|>cu, d_speed|>cu, d_neighbors|>cu, W₁, W₂, b₁, b₂, cm, x, 
out, avg_coord)
...
function update_kernel!(d_i, d_r, d_θ, d_τ, d_ϕ, d_cn, d_sx, d_rr, d_coords, d_speed, d_neighbors, W₁, W₂, b₁, b₂, cm, x, out, avg_coord)
    ...
    for jdx in 1:length(d_i)
        if idx != jdx
            ...
            #Maybe this is not necessary, cause now we have the reproduction radius. For the 
            # interaction neighborhood, it takes the form of a Gaussian instead of a solidly defined 
            #neighborhood
#             if dist < d_r
#                 nei+=1
#                 if nei<=size(d_neighbors)
#                     @inbounds d_neighbors[nei,idx] = jdx
#                 end
#             end
...
#     end
    ...
    for jdx in 1:length(d_i)
        ...
        #The dynamics for the FITNESS is missing, you got to make it!!!!
        ...
        if dist < d_rr[idx] && d_sx[idx] != d_sx[jdx]
            ...
            #BUG the problem lies here I dont know why...
            ##################################################
