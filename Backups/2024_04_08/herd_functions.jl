...
function get_direction(parameters_dict::Dict, boid₁::Boid, Boids::Vector{Boid})
    dist = []
    ...
    for boid₂ in Boids
        if boid₁ != boid₂
            ...
            if vision == true
                fov = (1.0 + cos(boid₁.θ - boid₂.θ)) #Field of vision
            else
                fov = 1.0
            end

            center_of_mass .+= ((exp(-β*dist^2.0)/length(Boids)) * fov) .* boid₂.pos
            average_θ += ((exp(-β*dist^2.0)/length(Boids)) * fov) * boid₂.θ
        end
    end

    # Prepare input data for the model
    input_positions = vcat(center_of_mass...)
    input_data = cat(input_positions,average_θ,dims=1)

    # Forward pass through the model
    angle = π*boid₁.model(input_data)[1]
...
function update_boids!(Boids::Vector{Boid}, parameters_dict::Dict, Δt::Float64)
    ...
    nₚₐᵣₜₛ = parameters_dict["maxₚₐᵣₜₛ"]
...
#     @cuda threads=length(Boids) update_kernel!(d_i|>cu, d_r|>cu, d_τ|>cu, d_ϕ|>cu, d_cn|>cu, d_sx|>cu, d_rr|>cu, d_pos|>cu, d_angle|>cu, d_model)#, d_speed, d_neighbors, d_params)
    @cuda threads=length(Boids) update_kernel!(d_i|>cu, d_r|>cu, d_τ|>cu, d_ϕ|>cu, d_cn|>cu, d_sx|>cu, d_rr|>cu, d_coords|>cu, d_speed|>cu, d_params|>cu, W₁, W₂, b₁, b₂, cm, x, out, avg_coord)
#
#     # Transfer updated data back to CPU memory
#     CUDA.copyto!(updated_Boids, d_updated_Boids)
end

function update_kernel!(d_i, d_r, d_τ, d_ϕ, d_cn, d_sx, d_rr, d_coords, d_speed, d_params, W₁, W₂, b₁, b₂, cm, x, out, avg_coord)
...
#     #         @inbounds dist = norm(d_coords[1:2,idx] .- d_coords[1:2,jdx])
#     #         @inbounds fov = (1.0 + cos(d_coords[3,idx] - d_coords[3,jdx])) #Field of vision
#             #d_neighbors[idx] = jdx
#     #         avg_coord .+= ((exp(-β*dist^2.0)/length(d_i)) * fov) .* d_coords[jdx]
#             @inbounds dist = norm(d_coords[idx] .- d_coords[jdx])
#             x .+= ((exp(-β*dist^2.0)/length(d_i))) .* d_coords[jdx]
        end
    end
    ...
    @inbounds out = W₂[idx] * (tanh.(W₁[idx] * x .+ b₁[idx])) .+ b₂[idx]


#
        #Ok, so this works, now I need to make everything calculate the average
        #And make the Neural Model work properly
#         @inbounds out .= W₂[idx] .+ tanh.(W₁[idx]) .+ dist .+ b₁[idx] .+ b₂[idx]

#         # I NEED TO FIND A WAY TO PUT CM AND AVG_θ AS INPUTS FOR THE NN MODEL
#
#         # x is the input of the network
#         # z2 is the output of the network
# #         @inbounds z1 = W₁[idx] * x + b₁[idx]
# #         a1 = tanh.(z1)
# #         @inbounds z2 = W₂[idx] * a1 + b₂[idx]
#
#         @inbounds out .= W₂[idx] * (tanh.(W₁[idx] * dist .+ b₁[idx])) .+ b₂[idx]
#         @inbounds out .= W₂[idx] * ((W₁[idx] * dist .+ b₁[idx])) .+ b₂[idx]



        #Here I need to reconstruct the model
...
#         angle[idx] = mod(angle[idx] + get_direction())
#
#         dist = []
#
#        β = parameters_dict["β"]
...
#         if innertia
#             boid₁.θ = mod(updated_Boids[idx].θ + get_direction(boid₁, Boids) * Δt + π, 2.0 * π) - π
#         else
#             updated_Boids[idx].θ = get_direction(parameters_dict, boid₁, Boids)
#         end
#
